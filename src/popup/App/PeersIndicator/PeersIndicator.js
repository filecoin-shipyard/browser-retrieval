...
import StatusIndicator from 'src/popup/components/StatusIndicator';

function PeersIndicator(props) {
  ...
  return (
    <StatusIndicator
      ...
      badge={peersState && peersState.length}
      label={peersState ? 'Connected to:' : 'Not connected to any peers yet'}
      description={peersState && (peersState.join('\n') || 'no peer')}
      {...props}
    />
  );
}
